...
public class ModelUtils {
    ...
    /**
     * Replaces the standard {@link EFactory} for an {@link EPackage} with the
     * special {@link ModelEFactory} which handles creation of dynamic eobjects
     * in a better way.
     * 
     * @param modelPackage
     * @param ePackage
     */
    public static void setEFactoryProxy(final ModelPackage modelPackage, final EPackage ePackage) {
     ...
     */
    public static void setXMLDateFormat(final SimpleDateFormat simpleDateFormat) {
    ...
    /**
     * Return true if the {@link EStructuralFeature} is unsettable, non required
     * and has no default.
     */
    public static boolean isUnsettable(final EStructuralFeature eFeature) {
     ...
     */
    public static boolean isAnyType(final EStructuralFeature eFeature) {
    ...
    /**
     * If the value is a featuregroup then walk through the structure to find
     * the deepest one and return that value.
     */
    public static Object findValue(final ModelFeatureMapEntry<?> modelFeatureMap) {
        if (FeatureMapUtil.isFeatureMap(modelFeatureMap.getEStructuralFeature())) {
            final ModelFeatureMapEntry<?> modelFeatureMapEntry = ModelResolver.getInstance().getModelFeatureMapEntry(modelFeatureMap.getEStructuralFeature(),
                    modelFeatureMap.getValue());
     ...
     * the deepest one and return that value.
     * 
     * In EMF nested featuremaps are stored in a flat list, in Texo the
     * hierarchical structure is maintained this method helps in the conversion.
     */
    public static EStructuralFeature findFeature(final ModelFeatureMapEntry<?> modelFeatureMap) {
        if (FeatureMapUtil.isFeatureMap(modelFeatureMap.getEStructuralFeature())) {
            final ModelFeatureMapEntry<?> modelFeatureMapEntry = ModelResolver.getInstance().getModelFeatureMapEntry(modelFeatureMap.getEStructuralFeature(),
                    modelFeatureMap.getValue());
    ...
    /**
     * Returns a qualified string representation of the
     * {@link EStructuralFeature} using
     * {@link #getQualifiedNameFromEClass(EClass)}.
     */
    public static String getQualifiedNameFromEStructuralFeature(final EStructuralFeature eFeature) {
        return getQualifiedNameFromEClass(eFeature.getEContainingClass()) + QUALIFIERSEPARATOR + eFeature.getName();
    }

    /**
     * Returns the {@link EStructuralFeature} of a certain {@link EClass} also
     * encoded in the name.
     * 
     * @param name
     * @see #getEClassFromQualifiedName(String)
     */
    public static EStructuralFeature getEStructuralFeatureFromQualifiedName(final String name) {
    ...
    /**
     * Returns a qualified string representation of the class using the ns
     * prefix of the epackage.
     */
    public static String getQualifiedNameFromEClass(final EClass eClass) {
        return eClass.getEPackage().getNsPrefix() + QUALIFIERSEPARATOR + eClass.getName();
    }

    /**
     * Searches all the eclassifiers of the {@link ModelPackage} instances that
     * have been registered with the {@link ModelResolver}.
     * 
     * Note, can handle qualified as well as unqualified names. The qualified
     * names will have the epackage ns prefix prepended separated by the
     * {@link #QUALIFIERSEPARATOR}, for example: library|Book
     * 
     * But also unqualified names are handled, although then if different
     * epackages have eclasses with the same name then the wrong eclass can be
     * returned.
     * 
     * @param name
     ...
     */
    public static EClass getEClassFromQualifiedName(final String name) {
     ...
     * @see #getEClassFromQualifiedName(String)
     */
    public static EClassifier getEClassifierFromQualifiedName(final String name) {
    ...
    /**
     * Searches for an {@link EPackage} available in the {@link ModelResolver}
     * or the {@link XMLTypePackage} or the {@link EcorePackage}. It first tries
     * to match the namespace uri, then the name and finally the namespace
     * prefix.
     * 
     * @param identifier
     *            , the uri, name or namespace prefix of the {@link EPackage} to
     *            search for
     * @return a found {@link EPackage}
     * @throws IllegalArgumentException
     *             if no {@link EPackage} can be found
     */
    public static EPackage getEPackageFromNameUriOrPrefix(final String identifier) {
        final List<EPackage> toSearch = new ArrayList<>();
    ...
    /**
     * Returns the lower case version of the string converted with English
     * Locale
     **/
    ...
    /**
     * Returns the upper case version of the string converted with English
     * Locale
     **/
    ...
    /**
     * Returns the first char only lower case version of the string converted
     * with English Locale
     **/
    ...
    /**
     * Returns the first char only upper case version of the string converted
     * with English Locale
     **/
    public static String upCaseFirst(final String value) {
        if (value == null) {
            return null;
        }
    ...
    /**
     * Converts a XML String date(time) to a Date object. Note: method is
     * synchronized as it uses a shared {@link SimpleDateFormat} instance.
     * 
     * @param xmlString
     *            the xml String containing the date time in XML Schema format.
     * @return a java Date object representing the date time coded in the
     *         xmlString
     */
    ...
    /**
     * Converts a Date to a valid XML String. Note: method is synchronized as it
     * uses a shared {@link SimpleDateFormat} instance.
     * 
     * @param dt
     *            the Date to format
    ...
    /**
     * Read the ecore file belonging to the {@link ModelPackage}
     * ({@see ModelPackage#getEcoreFileName()}. The default impl. will read the
     * ecore file from the classpath of the passed ModelPackage . All read
     * EPackages are registered in the central {@link EPackage.Registry} from
    ...
    /**
     * Recursively registers an EPackage and its subpackages. If an ePackage was
     * already registered then that one is returned.
     * 
     * @param the
     *            ePackage to register
     * @return the ePackage found in the registry or the passed ePackage
     */
    public static EPackage registerEPackage(final EPackage ePackage, final EPackage.Registry registry) {
        ...
        for (final EPackage eSubPackage : new ArrayList<>(ePackage.getESubpackages())) {
     ...
     * @return
     */
    public static EEnum getEnumBaseDataTypeIfObject(final EDataType eDataType) {
        if (eDataType instanceof EEnum) {
            return null;
        }
        final ExtendedMetaData extendedMetaData = ExtendedMetaData.INSTANCE;
        for (EDataType baseDataType = eDataType; baseDataType != null; baseDataType = extendedMetaData.getBaseType(baseDataType)) {
    ...
    /**
     * True if the eDataType is an EEnum or has as instance class the
     * {@link Enumerator}.
     */
    public static boolean isEEnum(final EDataType eDataType) {
    ...
    public static boolean isEMap(final EClass eClass) {
        return eClass != null && eClass.getInstanceClass() != null && Map.Entry.class.isAssignableFrom(eClass.getInstanceClass())
                && eClass.getEStructuralFeature("key") != null //$NON-NLS-1$
    ...
    public static boolean isMixed(final EStructuralFeature feature) {
    ...
    public static boolean hasMixedEFeature(final EClass eClass) {
    ...
    /**
     * Returns true if the {@link EModelElement} has an {@link EAnnotation} with
     * source {@link ModelConstants#EANNOTATION_SOURCE} and the passed in
     * annotionKey.
     * 
     * @param eModelElement
     ...
     */
    public static boolean hasEAnnotation(final EModelElement eModelElement, final String annotationKey) {
        return null != getEAnnotation(eModelElement, annotationKey);
    }

    /**
     * Returns the value of the EAnnotation of the {@link EModelElement} with
     * source {@link ModelConstants#EANNOTATION_SOURCE} and as the key the
     * annotionKey.
     * 
     * @param eModelElement
     ...
     */
    public static String getEAnnotation(final EModelElement eModelElement, final String annotationKey) {
    ...
    /**
     * Temp uris are used to correctly handle references when inserting new
     * eobjects.
     * 
     * @see EObjectStore
     */
    public synchronized static URI makeTempURI(final URI baseURI) {
        TEMP_COUNTER++;
    ...
    /**
     * Is the uri string a temporary uri which needs to be handled differently
     * than other uris.
     */
    public static boolean isTempURI(final String baseURI) {
        return baseURI.contains(TEMP_URI_CODE + "="); //$NON-NLS-1$
    }

    /**
     * Convert a web service uri for an object to an EMF uri. EMF uri's use the
     * fragment for the object identifier while web service uri's will have the
     * the identifier encoded in the segments.
     * 
     * example: http://example.com#Book||1 http://exampke.com/Book/1
     */
    public static URI convertToEMFURI(final URI webServiceURI) {
        final TypeIdTuple tuple = getTypeAndIdFromUri(true, webServiceURI);
        final URI baseURI = webServiceURI.trimSegments(2);
        return baseURI.appendFragment(ModelUtils.getQualifiedNameFromEClass(tuple.getEClass()) + ModelConstants.FRAGMENTSEPARATOR + tuple.getId());
    }

    /**
     * @see #convertToEMFURI(URI)
     */
    public static URI convertToWebServiceURI(final URI emfURI) {
        final TypeIdTuple tuple = getTypeAndIdFromUri(true, emfURI);
        final URI baseURI = emfURI.trimFragment();
        return baseURI.appendSegment(ModelUtils.getQualifiedNameFromEClass(tuple.getEClass())).appendSegment(tuple.getId() + "").appendFragment(""); //$NON-NLS-1$ //$NON-NLS-2$
    }

    public static TypeIdTuple getTypeAndIdFromUri(final boolean useWebServiceFormat, final URI objectUri) {
        EClass eClass;
        Object idValue;
