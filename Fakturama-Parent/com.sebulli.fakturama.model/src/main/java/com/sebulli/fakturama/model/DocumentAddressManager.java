...
public class DocumentAddressManager implements IDocumentAddressManager {
     ...
     * {@link BillingType}. Nearly all fields are copied from {@link Contact}
     * except
     * <code>manualAddress</code>.
     * 
     * @param contact
     *            the {@link Contact} to copy
     * @param billingType
     *            {@link BillingType} of {@link DocumentReceiver}
     * @return {@link DocumentReceiver}
     */
    @Override
    public DocumentReceiver createDocumentReceiverFromAddress(final Address address, final BillingType billingType) {
        final Contact contact = address.getContact();
        return createDocumentReceiver(contact, address, billingType);
    }

    private DocumentReceiver createDocumentReceiver(final Contact contact, final Address address, final BillingType billingType) {
        final DocumentReceiver documentReceiver = modelFactory.createDocumentReceiver();
    ...
    @Override
    public DocumentReceiver createDocumentReceiverForBillingType(final Contact contact, final BillingType billingType) {
        final Optional<Address> addressFromContact = getAddressFromContact(contact, billingType.isINVOICE() ? ContactType.BILLING : ContactType.DELIVERY);
        // copy address data
        return createDocumentReceiver(contact, addressFromContact.orElse(null), billingType);
    }

    @Override
    public Optional<Address> getAddressFromContact(final Contact contact, final ContactType contactType) {
        Optional<Address> address = Optional.empty();
        if (contact != null && contactType != null) {
            address = contact.getAddresses().stream().filter(rcv -> rcv.getContactTypes().isEmpty() || rcv.getContactTypes().contains(contactType)).findFirst();
    ...
    @Override
    public DocumentReceiver getBillingAdress(final Document document) {
        return getAdressForBillingType(document, BillingType.INVOICE);
    }

    @Override
    public DocumentReceiver getDeliveryAdress(final Document document) {
        return getAdressForBillingType(document, BillingType.DELIVERY);
    }

    @Override
    public DocumentReceiver getAdressForBillingType(final Document document, final BillingType billingType) {
        ...
        Optional<DocumentReceiver> documentReceiver = document.getReceiver().stream().filter(rcv -> rcv.getBillingType().equals(filterBillingType)).findFirst();

        // FALLBACK: Use billing type of the given document
        if (!documentReceiver.isPresent()) {
            documentReceiver = document.getReceiver().stream().filter(rcv -> rcv.getBillingType().equals(document.getBillingType())).findFirst();
        }
        return documentReceiver.orElse(null);
    }

    @Override
    public Document addOrReplaceReceiverToDocument(final Document document, final DocumentReceiver documentReceiver) {
        final Optional<DocumentReceiver> existingReceiver = document.getReceiver().stream()
                .filter(r -> r.getBillingType() == documentReceiver.getBillingType()).findAny();
