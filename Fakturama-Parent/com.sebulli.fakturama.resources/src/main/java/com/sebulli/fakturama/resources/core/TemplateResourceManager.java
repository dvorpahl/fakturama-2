...
/**
 * This is the "worker" class for creating the full template directory structure
 * in a (new) workspace. Called from ConfigurationManager.
 *
 */
public class TemplateResourceManager implements ITemplateResourceManager {
    ...
    private void resourceCopy(final String resource, final Path targetFile) {
        ...
        if (fileResource == null) {
            fileResource = FileLocator.find(definingBundle, new org.eclipse.core.runtime.Path("/target/classes/nl/de" + resource.replaceAll("/\\$nl\\$", "")),
                    null);
        }
     ...
     */
    public String translate(String key, final Object... args) {
        if (key == null) {
            return "";
        }
        if (key.charAt(0) != '%') {
