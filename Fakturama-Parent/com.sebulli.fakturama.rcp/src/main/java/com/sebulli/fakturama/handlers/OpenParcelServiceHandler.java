...
public class OpenParcelServiceHandler {
     ...
     */
    @Execute
    public void run(final Shell shell, final MApplication application, final EModelService modelService, @Active final MPart activePage) {
        final MPartStack documentPartStack = (MPartStack) modelService.find(CallEditor.DETAIL_PARTSTACK_ID, application);

        final BrowserEditor browserEditor = findActiveDocumentOrGetBrowser(application, modelService, activePage);
        ...
        if (dataSetDocument != null) {
            if (dataSetDocument instanceof Document) {
                ...
                if (parcelServiceManager.size() > 1) {
                    ...
                    final int x = shell.getDisplay().getCursorLocation().x;
                    final int y = shell.getDisplay().getCursorLocation().y;
            ...
            MessageDialog.openWarning(shell, msg.dialogMessageboxTitleInfo, msg.commandParcelserviceWarning);
        }
    }
    ...
    private BrowserEditor findActiveDocumentOrGetBrowser(final MApplication application, final EModelService modelService, final MPart activePage) {
        ...
        if (activePart == null || activePart.getElementId().contentEquals(NavigationView.ID)) {
            ...
            for (final MStackElement stackElement : documentPartStack.getChildren()) {
                if (stackElement.isVisible() && (stackElement.getElementId().contentEquals(DocumentEditor.ID))) {
            ...
            if (activePart == null) {
                ...
                for (final MStackElement stackElement : documentPartStack.getChildren()) {
                    if (stackElement.isVisible() && (stackElement.getElementId().contentEquals(ParcelServiceBrowserEditor.ID)
            ...
            if (activePart == null) {
                ...
                final List<MUIElement> dataPanelElements = modelService.findElements(application, DocumentsListTable.ID, MUIElement.class, null,
        ...
        if (documentEditor == null && activePart != null) {
            ...
            // A parcel service browser editor is active
                ...
                final AbstractViewDataTable currentListtable = (AbstractViewDataTable) activePart.getObject();
                final Document[] selectedObjects = (Document[]) currentListtable.getSelectedObjects();
