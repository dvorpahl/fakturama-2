...
public class WebShopStatusSettingsDialog extends TitleAreaDialog {
    ...
    @Inject
    public WebShopStatusSettingsDialog(@Named(IServiceConstants.ACTIVE_SHELL) final Shell shell) {
        super(shell);
    ...
    protected Control createDialogArea(final Composite parent) {
        ...
        Composite container = CompositeFactory.newComposite(SWT.NONE)
                ...
                .layoutData(GridDataFactory.fillDefaults().create()).create(area);

        setMessage(msg.preferencesWebshopSettingsDescription, IMessageProvider.INFORMATION);

        ButtonFactory.newButton(SWT.PUSH).layoutData(GridDataFactory.swtDefaults().align(SWT.LEFT, SWT.TOP).span(2, 1).create())
                .text(msg.preferencesWebshopSettingsGetallstates).onSelect(e -> {
                    Webshopexport wsExport = execute(parent.getShell());
                    if (wsExport != null) {
                        // set the values from web shop
                        shopInfoString.setText(String.format("%s (%s: %s)", wsExport.getWebshop().getShop(), msg.helpAboutfeaturesdialogVersion,
        ...
        LabelFactory.newLabel(SWT.READ_ONLY).text(msg.preferencesWebshopSettingsShopinfo).create(container);
        shopInfoString = TextFactory.newText(SWT.READ_ONLY).layoutData(GridDataFactory.fillDefaults().grab(true, false).create()).create(container);

        LabelFactory.newLabel(SWT.READ_ONLY).text(msg.preferencesWebshopSettingsVersion).create(container);
        connectorVersion = TextFactory.newText(SWT.READ_ONLY).layoutData(GridDataFactory.fillDefaults().grab(true, false).create()).create(container);
    ...
    private void createTreeMapperWidget(final Composite parent) {
        ...
        ISemanticTreeMapperSupport<WebshopOrderStateMapping, WebshopOrderState, OrderState> semanticSupport = new ISemanticTreeMapperSupport<>() {
            @Override
            public WebshopOrderStateMapping createSemanticMappingObject(final WebshopOrderState leftItem, final OrderState rightItem) {
            ...
            @Override
            public WebshopOrderState resolveLeftItem(final WebshopOrderStateMapping semanticMappingObject) {
                return semanticMappingObject.getLeftItem();
            }

            @Override
            public OrderState resolveRightItem(final WebshopOrderStateMapping semanticMappingObject) {
    ...
    private boolean createMappings() {
        ...
        if (!missingMappings.isEmpty()) {
            creationOk = MessageDialog.openQuestion(getShell(), msg.dialogMessageboxTitleWarning, msg.preferencesWebshopSettingsAssignstateMissing);
            if (!creationOk) {
                return creationOk;
            }
        }
    ...
    /**
     * Sets the input to the TreeMapper widget and activates it. The Widget can
     * only be activated if mappings are available. Otherwise it remains
     * disabled.
     *
     * @param leftTreeInput
     *            the left tree input
     */
    private void setInputAndActivateTreeMapperWidget(final List<WebshopOrderState> leftTreeInput) {
     ...
     * @param leftTreeInput
     *            the left tree input
     * @param forceReMapping
     *            if we want to throw away any old (or incomplete) mapping
     */
    private void setInputAndActivateTreeMapperWidget(final List<WebshopOrderState> leftTreeInput, final boolean forceReMapping) {
        ...
        if (!webshopOrderStatesMapping.isEmpty()) {
            for (WebshopStateMapping webshopStateMapping : webshopOrderStatesMapping) {
                WebshopOrderState webshopOrderState = new WebshopOrderState(webshopStateMapping.getWebshopState(), webshopStateMapping.getName());
                leftTreeInput.add(webshopOrderState);
    ...
    /**
     * This is kind of a DTO. This class holds the mappings between a web shop
     * state to the Fakturama {@link OrderState}.
     * 
     */
    public class WebshopOrderStateMapping {
         ...
         *
         * @param left
         *            the remote {@link WebshopOrderState}
         * @param right
         *            the {@link OrderState} to which the
         *            {@link WebshopOrderState} was mapped by.
         */
        public WebshopOrderStateMapping(final WebshopOrderState left, final OrderState right) {
            this.leftItem = left;
         ...
         *            the left item
         */
        public final void setLeftItem(final WebshopOrderState leftItem) {
         ...
         *            the right item
         */
        public final void setRightItem(final OrderState rightItem) {
    ...
    public class WebshopOrderState {
        ...
        public WebshopOrderState(final String id, final String text) {
            this.id = id;
            this.stateName = text;
        }
     ...
     * @param parent
     */
    public Webshopexport execute(final Shell parent) {
        ...
        WebShopConnector conn = new WebShopConnector().withScriptURL(StringUtils.prependIfMissingIgnoreCase(shopURL, "http://", "https://", "file://"))
