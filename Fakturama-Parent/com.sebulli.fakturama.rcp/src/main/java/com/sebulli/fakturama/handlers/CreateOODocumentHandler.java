...
public class CreateOODocumentHandler {
     ...
     * </p>
     * <p>
     * If a {@link Document} is given and the handler is called with "silent
     * mode" the template selection dialog and all other dialogs are suppressed
     * and the {@link Document} is printed immediately. You have to provide a
     * template in this case.
     * </p>
     */
    @Execute
    public void run(final Shell shell, final EPartService partService, @Optional @Named(PARAM_DOCUMENT) final String docId,
            ...
            if (StringUtils.equalsIgnoreCase(activePart.getElementId(), DebitorEditor.ID)) {
                ...
                if (documentEditor != null) {
                    ...
                    if (templates.size() > 1) {
                        ...
                        MessageDialog.openWarning(shell, msg.dialogMessageboxTitleInfo, MessageFormat.format(msg.dialogPrintooNotemplate,
                                StringUtils.join(templateFinder.getLocalizedRelativeFolder(documentEditor.getDocumentType()), File.separatorChar)));
                    }
                } else {
    ...
    /**
     * <p>
     * The stock update works as follows:
     * </p>
     * <p>
     ...
     * <li>deleted</li>
     * </ul>
     * {@link DocumentItem}s. For this, we hold the previous
     * {@link DocumentItem} list and compare it to the current one. Here we can
     * find the updated {@link DocumentItem}s (which are available in both
     * lists), the new {@link DocumentItem}s (which are only available in the
     * current list) and the deleted {@link DocumentItem}s (which are only
     * available in the old list). The resulting list is a combination of the
     * changed and created {@link DocumentItem}s from the current list and the
     * deleted {@link DocumentItem}s from the old list. This resulting list
     * contains {@link DocumentItem}s which each contains a transient field (not
     * stored to database) named <code>quantityOrigin</code>. This field is
     * filled like follows:
     * <ul>
     * <li>changed {@link DocumentItem}s &rArr; <code>quantityOrigin</code>
     * contains the old <code>quantity</code>
     * <li>new {@link DocumentItem}s &rArr; <code>quantityOrigin</code> contains
     * nothing (<code>null</code>)
     * <li>deleted {@link DocumentItem}s &rArr; <code>quantityOrigin</code>
     * contains the old <code>quantity</code>, but the <code>quantity</code> of
     * this {@link DocumentItem} is set to <code>null</code>.</li>
     * </p>
     * <p>
     * <b><i>This merged list is only for checking the changed, created or
     * deleted {@link DocumentItem}s and is NOT intended to being stored in
     * database!</i></b>
     * </p>
     * 
     * @param shell
     ...
     */
    private void updateStockQuantity(final Shell shell, final List<DocumentItem> olditemsList, final Document currentDocument) {
        ...
        for (DocumentItem currentDocumentItem : currentDocument.getItems()) {
            ...
            for (DocumentItem oldItem : olditemsList) {
                ...
                if (StringUtils.equalsIgnoreCase(oldItem.getItemNumber(), currentDocumentItem.getItemNumber())) {
                    ...
                    if (currentDocumentItem.getQuantity() != null && currentDocumentItem.getQuantity().compareTo(oldItem.getQuantity()) != 0) {
    ...
    private boolean isStockUpdateForCurrentDocument(final Document currentDocument) {
     ...
     * @param itemToAdd
     */
    private void addToTmpItems(final Map<Integer, DocumentItem> tmpDocItems, final DocumentItem itemToAdd) {
        if (tmpDocItems.get(itemToAdd.hashCode()) != null) {
            itemToAdd.setOriginQuantity(itemToAdd.getOriginQuantity() + tmpDocItems.get(itemToAdd.hashCode()).getQuantity());
        }
