...
public class VatsDAO extends AbstractDAO<VAT> {
    ...
    @Override
    protected Set<Predicate> getRestrictions(final VAT object, final CriteriaBuilder cb, final Root<VAT> vat) {
     ...
     * @return
     */
    public VAT findByOldVat(final OldVats oldVat) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<VAT> criteria = cb.createQuery(VAT.class);
        Root<VAT> root = criteria.from(VAT.class);
        criteria.where(cb.and(cb.equal(root.get(VAT_.description), oldVat.getDescription()), cb.equal(root.get(VAT_.name), oldVat.getName())));
    ...
    public long countByCategory(final AbstractCategory cat) {
        ...
        criteria.select(cb.count(root)).where(cb.and(cb.equal(root.get(VAT_.category), cat), cb.isFalse(root.get(VAT_.deleted))));
        return getEntityManager().createQuery(criteria).getSingleResult();
    }
     ...
     */
    public List<VAT> findVATPreferredCategory(final String category) {
        VATCategory vATCategory = vatCategoriesDAO.findCategoryByName(category);
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<VAT> criteria = cb.createQuery(VAT.class);
        Root<VAT> root = criteria.from(VAT.class);
        criteria.where(cb.or(cb.equal(root.get(VAT_.category), vATCategory), cb.isNull(root.get(VAT_.category))));
    ...
    public List<VAT> findNoVATEntries() {
        ...
        CriteriaQuery<VAT> criteria = cb.createQuery(VAT.class);
        Root<VAT> root = criteria.from(VAT.class);
        criteria.where(cb.and(cb.equal(root.get(VAT_.taxValue), Double.valueOf(0.0)),
                cb.or(cb.equal(root.get(VAT_.category), vATCategory), cb.isNull(root.get(VAT_.category))))
