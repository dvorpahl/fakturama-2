...
public class ReceiptVouchersDAO extends AbstractDAO<Voucher> {
    ...
    public List<Voucher> findAll(final boolean forceRead) {
        ...
        TypedQuery<Voucher> query = getEntityManager()
                .createQuery(criteria.where(cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.RECEIPTVOUCHER))));
        if (forceRead) {
     ...
     */
    public List<AccountEntry> findAccountedReceiptVouchers(final VoucherCategory account) {
     ...
     * @return List of {@link AccountEntry}s, sorted by {@link Voucher} date
     */
    public List<AccountEntry> findAccountedReceiptVouchers(final VoucherCategory account, final Date startDate, final Date endDate) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
    ...
    public String[] getVoucherNames(final int maxLength) {
        ...
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        Predicate predicate = cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.RECEIPTVOUCHER),
