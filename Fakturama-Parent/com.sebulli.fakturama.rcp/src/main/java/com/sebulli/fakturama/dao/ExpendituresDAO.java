...
public class ExpendituresDAO extends AbstractDAO<Voucher> {
    ...
    /**
     * The following attributes are compared: name, category, date, nr,
     * documentnr.
     * 
     * @see com.sebulli.fakturama.dao.AbstractDAO#getRestrictions(java.lang.Object,
     *      javax.persistence.criteria.CriteriaBuilder,
     *      javax.persistence.criteria.Root)
     */
    @Override
    protected Set<Predicate> getRestrictions(final Voucher object, final CriteriaBuilder criteriaBuilder, final Root<Voucher> root) {
        ...
        if (object.getAccount() != null) {
            restrictions.add(criteriaBuilder.equal(root.get(Voucher_.account).get(VoucherCategory_.name), object.getAccount().getName()));
        }
     ...
     */
    @Override
    public List<Voucher> findAll(final boolean forceRead) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        TypedQuery<Voucher> query = getEntityManager()
                .createQuery(criteria.where(cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.EXPENDITURE))));
        if (forceRead) {
    ...
    /**
     * Finds Vouchers having a given account. An account is a
     * {@link VoucherCategory}.
     * 
     * @param account
     *            which account should be used for filtering
     * @return List of {@link AccountEntry}s, sorted by Voucher date
     */
    public List<AccountEntry> findAccountedExpenditures(final VoucherCategory account) {
        return findAccountedExpenditures(account, null, null);
    }

    /**
     * Finds {@link Voucher}s having a given account. An account is a
     * {@link VoucherCategory}. The {@link Voucher}s can be filtered for a
     * certain date range.
     * 
     * @param account
     *            which account should be used for filtering
     ...
     */
    public List<AccountEntry> findAccountedExpenditures(final VoucherCategory account, final Date startDate, final Date endDate) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        Predicate predicate = cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.account), account),
        ...
        if (startDate != null && endDate != null) {
            ...
            predicate = cb.and(predicate, cb.between(root.get(Voucher_.voucherDate), startDate.before(endDate) ? startDate : endDate,
    ...
    /**
     * Finds all undeleted {@link Voucher}s sorted by {@link ItemAccountType}
     * and date.
     * 
    ...
    public List<Voucher> findAllExpendituresSorted() {
        ...
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        Predicate predicate = cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.EXPENDITURE));
        CriteriaQuery<Voucher> cq = criteria.where(predicate).orderBy(cb.asc(root.get(Voucher_.account)), cb.asc(root.get(Voucher_.voucherDate)));
        TypedQuery<Voucher> query = getEntityManager().createQuery(cq);
        return query.getResultList();
    ...
    /**
     * Finds all Vouchers within a given date range. If one of the dates (or
     * both) is <code>null</code> then all Vouchers will be in the returned
     * List.
     * 
     * @param startDate
     *            start of date range
     ...
     */
    public List<Voucher> findVouchersInDateRange(final GregorianCalendar startDate, final GregorianCalendar endDate) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        Predicate predicate = cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.EXPENDITURE));
        if (startDate != null && endDate != null) {
            // if startDate is after endDate we switch the two dates silently
            predicate = cb.and(predicate, cb.between(root.get(Voucher_.voucherDate), startDate.before(endDate) ? startDate.getTime() : endDate.getTime(),
    ...
    /**
     * Get all {@link Voucher} names as String array. Used for content
     * proposals.
     * 
     * @param maxLength
     *            max count of result items
     * @return array of voucher names.
     */
    public String[] getVoucherNames(final int maxLength) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<Voucher> criteria = cb.createQuery(getEntityClass());
        Root<Voucher> root = criteria.from(getEntityClass());
        Predicate predicate = cb.and(cb.not(root.get(Voucher_.deleted)), cb.equal(root.get(Voucher_.voucherType), VoucherType.EXPENDITURE),
    ...
    public String[] getVisibleProperties() {
        return new String[] { Voucher_.doNotBook.getName(), Voucher_.voucherDate.getName(), Voucher_.voucherNumber.getName(), Voucher_.documentNumber.getName(),
                Voucher_.name.getName(), Voucher_.paidValue.getName() };
    }
}
