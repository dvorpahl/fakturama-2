...
/**
 * Configures the application while startup. It checks for a new Workspace or
 * the need of database conversion. Furthermore, it checks if all the templates
 * are available and if the directory structure is ok. If needed, the templates
 * are copied from resource bundle into the template directory.
 * 
 */
public class ConfigurationManager {
        ...
        try {
            ...
            if (cmd.hasOption('w')) {
                ...
                if (Files.exists(workspacePath, LinkOption.NOFOLLOW_LINKS)) {
                    ...
                    log.warn("The requested workspace folder is invalid. Please select " + "a new one from the Fakturama menu (File - select workspace...). "
     ...
     */
    private void adaptLogfile(final String oldWorkspace, final String requestedWorkspace) {
     ...
     * @param shell
     */
    private void selectWorkspace(final String requestedWorkspace) {
    ...
    /**
     * Initialize the workspace. E.g., creates a new template folder if it not
     * exists and fills it with all the templates.
     * 
     * @param workspace
     *            the workspace path to use
     * 
     */
    private void initWorkspace(final String requestedWorkspace) {
