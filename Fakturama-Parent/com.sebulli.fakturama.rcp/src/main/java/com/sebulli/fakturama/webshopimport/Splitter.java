 ...
 * it hits a order element, it creates a new unmarshaller and unmarshals one
 * order.
 * 
 * <p>
...
public class Splitter extends XMLFilterImpl {
    ...
    public Splitter(final JAXBContext context, final IProgressMonitor localMonitor, final Path logFile) {
    ...
    public void startElement(final String namespaceURI, final String localName, final String qName, final Attributes atts) throws SAXException {
        ...
        if (!localMonitor.isCanceled()) {
            ...
            if (namespaceURI.equals("http://www.fakturama.org") && (localName.equals("product") || localName.equals("order"))) {
    ...
    private void logXml(final Object jaxbElement) {
        ...
        if (depth != 0) {
            ...
            if (depth == 0) {
                ...
                try {
                    switch (localName) {
                    case "product":
                        JAXBElement<ProductType> result = (JAXBElement<ProductType>) unmarshallerHandler.getResult();
                        process(result.getValue());
                        break;
                    case "order":
                        JAXBElement<OrderType> resultOrder = (JAXBElement<OrderType>) unmarshallerHandler.getResult();
    ...
    private void process(final OrderType value) {
        System.out.println("this order will be shipped at " + value.getDate());
    }

    private void process(final ProductType order) {
        System.out.println(String.format("current location: [%d:%d]", locator.getLineNumber(), locator.getColumnNumber()));
