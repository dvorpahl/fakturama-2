...
public class ExportPage extends ImportExportPage {
    ...
    @Inject
    public ExportPage(final IWorkbench workbench) {
    ...
    protected Composite createTreeViewer(final Composite parent) {
        ...
        exportTree = new CategorizedWizardSelectionTree(root, msg.wizardExportCommonFilterlabel);
    ...
    private IWizardRegistry getExportWizardRegistry() {
        if (exportWizardRegistry == null) {
            MAddon addon = application.getAddons().stream().filter(a -> a.getElementId().equals(ExportWizardRegistry.REGISTRY_ADDON_ID)).findFirst().get();
