 ...
 * 
 */
package com.sebulli.fakturama.money.osgi;
...
public class OsgiServiceProvider implements ServiceProvider {
    ...
    private <T> List<T> loadServices(final Class<T> serviceType) {
        ...
        try {
            ...
            Collection<ServiceReference<T>> serviceReferences = Activator.getContext().getServiceReferences(serviceType, null);

            services = serviceReferences.stream().map(sr -> Activator.getContext().getService(sr)).collect(Collectors.toList());
            @SuppressWarnings("unchecked")
            final List<T> previousServices = (List<T>) servicesLoaded.putIfAbsent(serviceType, (List<Object>) services);
            return Collections.unmodifiableList(previousServices != null ? previousServices : services);
        } catch (Exception e) {
            Logger.getLogger(OsgiServiceProvider.class.getName()).log(Level.WARNING, "Error loading services of type " + serviceType, e);
            return services;
        }
    }
}
