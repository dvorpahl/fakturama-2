...
public final class OsgiMonetaryCurrenciesSingletonSpi implements MonetaryCurrenciesSingletonSpi {
    ...
    @Override
    public Set<CurrencyUnit> getCurrencies(final CurrencyQuery query) {
        Set<CurrencyUnit> result = new HashSet<>();
        for (CurrencyProviderSpi spi : Bootstrap.getServices(CurrencyProviderSpi.class)) {
            try {
                result.addAll(spi.getCurrencies(query));
            } catch (Exception e) {
                Logger.getLogger(OsgiMonetaryCurrenciesSingletonSpi.class.getName()).log(Level.SEVERE,
                        "Error loading currency provider names for " + spi.getClass().getName(), e);
            }
        }
        return result;
    }
    ...
    /**
     * This default implementation simply returns all providers defined in
     * arbitrary order.
     *
