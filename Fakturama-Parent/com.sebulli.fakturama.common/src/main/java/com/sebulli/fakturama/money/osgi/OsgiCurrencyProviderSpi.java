...
public class OsgiCurrencyProviderSpi implements CurrencyProviderSpi {
    ...
    public OsgiCurrencyProviderSpi() {
        for (Currency jdkCurrency : Currency.getAvailableCurrencies()) {
            CurrencyUnit cu = CurrencyUnitBuilder.of(jdkCurrency.getCurrencyCode(), this.getProviderName()).build(true);
     ...
     *
     * @param currencyQuery
     *            the {@link javax.money.CurrencyContext} containing the
     *            parameters determining the query. not null.
     * @return the corresponding {@link CurrencyUnit}, or null, if no such unit
     *         is provided by this provider.
     */
    @Override
    public Set<CurrencyUnit> getCurrencies(final CurrencyQuery currencyQuery) {
    ...
    private CurrencyUnit getCurrencyUnit(final Locale locale) {
        Currency cur;
        try {
