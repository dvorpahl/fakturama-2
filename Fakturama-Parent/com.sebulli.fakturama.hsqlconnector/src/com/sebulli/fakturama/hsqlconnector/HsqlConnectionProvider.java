...
public class HsqlConnectionProvider implements IDbConnection, IActivateDbServer {
     ...
     */
    @Override
    public Properties activateServer(final Properties props) {
        server = new Server();
    ...
    public Connection getConnection() {
        ...
        try {
            ServiceReference<?>[] allServiceReferences = bundleContext.getAllServiceReferences(org.osgi.service.jdbc.DataSourceFactory.class.getName(),
            ...
            String url = String.format("jdbc:hsqldb:hsql://localhost:%d/%s", server.getPort(), server.getDatabaseName(0, false));
    ...
    public void stopServer() {
        if (isAlive()) {
            ...
            if (serviceReference == null) {
                throw new NullPointerException("no Service Found");
            }
